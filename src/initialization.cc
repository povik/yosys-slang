...
void finalize_variable_initialization(NetlistContext &netlist, ast::EvalContext &eval_context)
{
	visit_netlist_variables(netlist, [&](const ast::ValueSymbol &symbol) {
		...
		log_assert(storage);
		auto init_value = convert_const(*storage);
		...
		if (netlist.is_inferred_memory(*variable_symbol)) {
			...
			int abits = 32;
			ast_invariant(*variable_symbol, m->width * m->size == init_value.size());
			...
			bool little_endian = variable_symbol->getType().getFixedRange().isLittleEndian();
			meminit->setPort(
					ID::DATA, little_endian ? init_value : reverse_data(init_value, m->width));
			...
			for (int i = 0; i < init_value.size(); i++) {
				...
				if (!register_driven) {
					if (!driven) {
						cl.append(signal[i]);
						cr.append(init_value[i]);
					}
					// If not register driven, the init attribute
					// must be Sx
#if YOSYS_MAJOR == 0 && YOSYS_MINOR < 58
					init_value.bits()[i] = RTLIL::Sx;
#else
					init_value.set(i, RTLIL::Sx);
#endif
				}
			}
			...
			if (!init_value.is_fully_undef()) {
				...
				wire->attributes[ID::init] = init_value;
			}
